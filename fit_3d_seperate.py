##
# @file fit_3d_seperate.py
# @brief processing the optimization seperately( shape and pose ,here the betas is given) 
# @author Zhichao Zeng
# @version 1.0.0
# @date 2016-11-22
...
##
# @brief output the run time for initializing camera and optimizing pose params
#
# @param procession
# @param time
#
# @return 
def write_time( procession, time ):
    file = open( '../tmp/processingTime_seperate.txt','a+' )
    file.write(procession )
    file.write('  ')
    file.write( str( time ) )
    if procession == 'camera':
        file.write('   ')
    else:
        file.write( '\n' )
...
def optimize_on_joints(j2d,
                       ...
                       regs=None,
                       conf=None,
                       betas_dir='',           #the directory of betas
                       viz=False):
    ...
    if not exists( betas_dir ):
        print "betas_dir is not provite"
        raw_input( "run img2mod.py first " )
    params_file = open( betas_dir )
    params_from_file = pickle.load( params_file )
...
#        betas = ch.zeros(n_betas)
        betas =ch.array( params_from_file['betas'] )
...
def run_single_fit(img,
    ...
    t_camera_1 = time();
    tmp = t_camera_1 - t_camera_0;
    write_time( 'camera',tmp )
#    print 'time for fit the 3D to 2D is ',  tmp
#    print 'time for initializing camera is ' ,  tmp
#    raw_input('stop for show the time for estimating camera' )
    ...
    t_finish = time()
    tmp = t_finish - t_camera_1
    write_time( 'pose',tmp )
#    print 'time for fit the 3D to 2D is ',  tmp
#    raw_input('stop for showing the time of estimating camera' )
    h = img.shape[0]
    w = img.shape[1]
...
def main(base_dir,
    ...
    #_LOGGER.info( "img_dir is : " + img_dir )
    #_LOGGER.info( "data_dir is : " + data_dir )
    ...
    for ind, img_path in enumerate(img_paths):
        ...
        if exists(out_path):
            ...
            out_path = out_path.replace( '.pkl', 'pose.pkl' )
