...
# --------------------Camera estimation --------------------
    def guess_init(self, focal_length, j2d, init_pose):
        ...
        (_, A_global) = global_rigid_transformation(
            opt_pose, self.model.J, self.model.kintree_table, xp=ch)
    ...
    def initialize_camera(self,
        ...
        init_t = self.guess_init( flength, j2d, init_pose)
        t = ch.array(init_t)
    ...
    def optimize_on_joints(self,
        ...
        for o_id, orient in enumerate(orientations):
            ...
            sv = verts_decorated(
                ...
                betas=betas,
                shapedirs=self.model.shapedirs[:, :, :n_betas],
            ...
            Jdirs = np.dstack([self.model.J_regressor.dot(self.model.shapedirs[:, :, i])
                               for i in range(len(betas))])
            J_onbetas = ch.array(Jdirs).dot(betas) + self.model.J_regressor.dot(
                self.model.v_template.r)
    
            # get joint positions as a function of model pose, betas and trans
            (_, A_global) = global_rigid_transformation(
                sv.pose, J_onbetas, self.model.kintree_table, xp=ch)
            ...
            # added by Zhichao Zeng
            # iterm for differences between vertices of two frame's model
#            self.error = ( ch.sqrt( (( self.modelVertices - sv.r )**2).sum( axis = 1 ) ) ).sum( axis = 0 )
#            verticesError = lambda w : w *  self.error 
            verticesError = lambda w : w * ( ch.sqrt( (( self.modelVertices - sv.r )**2).sum( axis = 1 ) ) ).sum( axis = 0 )

            #iterm for defferences between pose of two frame's model
            if self.i > 1 :
                poseError = lambda w : w * ch.sqrt(((self.pose - sv.pose)**2).sum() )
            ...
            if regs is not None:
                ...
                sp = SphereCollisions(
                    pose=sv.pose, betas=sv.betas, model=self.model, regs=regs)
                sp.no_hands = True
            ...
            for stage, (w, wbetas) in enumerate(opt_weights):
                ...
                if self.i > 1:
                    ...
                    objs['verticesError'] = verticesError( 0.0005 )

                    objs['poseError'] = poseError( 0.001 )
    ...
    def run_single_fit(self,
        ...
        if self.i <= 1 :
            ...
            self.modelVertices = ch.array( self.model.r[:] )
            self.pose = ch.array( self.model.pose[:] )
        else :
            init_pose = self.pose
        if scale_factor != 1:
        ...
        (cam, try_both_orient, body_orient) = self.initialize_camera(
            j2d,
            img,
            init_pose,
            flength=flength,
        ...
        # fit
        (sv, opt_j2d, t) = self.optimize_on_joints(
            j2d,
            cam,
            img,
            prior,
        ...
        for deg in do_degrees:
            ...
            # now render
            im = (render_model(
                verts, self.model.f, w, h, cam, far=20 + dist) * 255.).astype('uint8')
            images.append(im)
    
        self.pose[:] = np.array( sv.pose[:] )
        self.i = self.i + 1
#        self.modelVertices[:] =np.array( self.model.r[:] )
        self.modelVertices[:] = np.array( sv.r )
