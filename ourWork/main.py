...
def call_fit_3d(img_dir,
    ...
    while True :
        file_name = raw_input( " input the image file name " )
        img_path = join( img_dir, file_name )
        out_path = '%s/%s.pkl' % ( out_dir, file_name)
        ...
        file_name = raw_input( 'input the est joints file name (npz file )' )


        #load 2D joints
        est = np.load( join( data_dir, file_name ) )['pose']
        joints = est[:2,:].T
        conf = est[2,:]
        ...
        if viz:
            ...
            if do_degrees is not None:
                for di, ind deg in enumerate(do_degrees):
                    plt.subplot(122)
                    plt.cla()
                    plt.imshow(vis[di])
                    plt.draw()
